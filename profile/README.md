...
- Focus on stability. Introduce only minimized out-of-tree tweaks:
    ...
    - `PropImitationHooks` is removed as it will prevents you from passing `Strong Integrity` and breaks `Device Integrity` if builtin fingerprint is banned by Google.
        - If you still wanna use it, go ahead and download variant with `PropImitationHOoks` variant (No guarantees for every build).
        - Still, You should use `PropImitationHooks` removed variant with [Play Integrity Fix](https://github.com/chiteroman/PlayIntegrityFix/) or [Tricky Store](https://github.com/5ec1cff/TrickyStore) instead!
